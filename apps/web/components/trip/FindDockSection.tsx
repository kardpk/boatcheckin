    ...
    <div className="mx-4 mt-3 bg-white rounded-[16px] border border-[#D0E2F3] p-5">
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="w-8 h-8 rounded-full bg-[#E8F2FB] flex items-center justify-center">
          <MapPin size={16} className="text-[#0C447C]" />
        </div>
        <p className="text-[17px] font-semibold text-[#0D1B2A]">{tr.findDock}</p>
      </div>

      {/* Marina info */}
      <p className="text-[15px] font-semibold text-[#0D1B2A]">{marinaName}</p>
      {marinaAddress && (
        <p className="text-[14px] text-[#6B7C93] mt-1">{marinaAddress}</p>
      )}

      {/* Slip badge */}
      {slipNumber && (
        <span className="inline-flex items-center mt-2 px-3 py-1 rounded-full bg-[#E8F9F4] text-[#1D9E75] text-[13px] font-medium">
          {tr.slip} {slipNumber}
        </span>
      )}
      ...
      <a
        ...
        className="mt-3 flex items-center gap-2 h-[44px] text-[14px] font-medium text-[#0C447C] hover:text-[#093a6b] transition-colors"
      >
      ...
      {/* Parking collapsible */}
      {parkingInstructions && (
        <div className="mt-3 border-t border-[#F5F8FC] pt-3">
      ...
      {/* Operating area */}
      {operatingArea && (
        <p className="mt-3 text-[13px] text-[#6B7C93]">
