...
export const BOAT_SPECIFIC_TOPICS: Record<string, BoatSpecificTopic[]> = {
  speedboat: [
    { key: "propeller_swim_platform", label: "Propeller Safety (CRITICAL)", emoji: "", defaultInstructions: "NEVER swim near the stern while the engine is running even when in neutral or idle. The propeller is EXTREMELY dangerous and can cause fatal injuries. The engine MUST be fully OFF before anyone enters the water or uses the swim ladder. The propeller danger zone extends 10 feet behind the transom. Wait for the captain's explicit 'ENGINE OFF' before entering the water.", compliance_target: "all" },
  ...
  center_console: [
    ...
    { key: "propeller_center_console", label: "Outboard Propeller Safety", emoji: "", defaultInstructions: "Center consoles are powered by outboard motor(s) with EXPOSED propellers at the stern. NEVER enter or exit the water near the engines. The engine MUST be OFF and in NEUTRAL before anyone enters the water. Do not stand on the swim platform while engines are running. The propeller danger zone extends 10 feet behind the transom.", compliance_target: "all" },
    { key: "sun_heat_exposure", label: "Sun & Heat Exposure", emoji: "", defaultInstructions: "Center consoles have limited shade. The combination of direct sun, reflected UV from the water, and salt spray can cause rapid dehydration and sunburn. Apply high-SPF lotion sunscreen (NOT spray slippery deck) BEFORE boarding and reapply every 90 minutes. Drink water continuously. Wear a hat, polarized sunglasses, and sun-protective clothing.", compliance_target: "all" },
  ...
  pwc: [
    ...
    { key: "propeller_strike_hazard", label: "Jet Intake Hazard", emoji: "", defaultInstructions: "Never put hands, feet, or hair near the jet intake at the rear. Hair entanglement can cause drowning.", compliance_target: "all" },
    ...
    { key: "off_throttle_steering", label: "Off-Throttle Steering Warning", emoji: "", defaultInstructions: "PWCs lose ALL steering ability when the throttle is released. Unlike a car, letting go does NOT stop you you will coast in a straight line with no directional control. Always maintain throttle through turns. If you see an obstacle, steer around it WITH throttle do not release.", compliance_target: "all" },
  ...
  pontoon: [
    ...
    { key: "propeller_zone", label: "Propeller Safety (CRITICAL)", emoji: "", defaultInstructions: "NEVER swim near the rear of the boat while the engine is running even in neutral. The propeller can cause FATAL injuries. Wait for the captain's explicit 'ENGINE OFF' confirmation before anyone enters the water. Use the swim ladder for re-boarding. The propeller danger zone extends 10 feet behind the transom.", compliance_target: "all" },
    ...
    { key: "weather_awareness", label: "Weather Awareness", emoji: "", defaultInstructions: "Be alert for rapidly changing weather conditions. If you see dark clouds, lightning, or a significant increase in wind, notify the captain immediately. Lightning on open water is extremely dangerous the boat is the tallest object. The captain will seek shelter or return to dock.", compliance_target: "all" },
    { key: "submarining", label: "Submarining Hazard", emoji: "", defaultInstructions: "Pontoons have low displacement bows. Shifting too much weight forward or plowing into a heavy wake can cause the bow to dive underwater (submarining), snapping the deck fence and swamping the boat. Keep weight distributed evenly. Slow down when crossing large wakes.", compliance_target: "bareboat_only" },
  ...
  fishing_charter: [
    ...
    { key: "prop_strike_fishing", label: "Propeller Strike Zone", emoji: "", defaultInstructions: "Never approach a person in the water with the engine in gear. The engine MUST be in NEUTRAL or OFF before anyone enters the water. A spinning prop creates suction and causes catastrophic amputations. The propeller danger zone extends 10 feet behind the transom.", compliance_target: "bareboat_only" },
  ],
  snorkel_dive: [
  ...
  houseboat: [
    ...
    { key: "generator_exhaust_stern", label: "Generator Exhaust Zone", emoji: "", defaultInstructions: "The generator exhaust exits near the stern/waterline. When the generator is running, the stern area is a CO DANGER ZONE. Do NOT sit on the swim platform, stand near the exhaust, or allow children to play near the back of the boat while the generator is running. CO accumulates in the 'station wagon effect' the area directly behind the boat.", compliance_target: "all" },
  ...
  other: [
    ...
    { key: "engine_propulsion_hazard", label: "Engine & Propulsion Hazard", emoji: "", defaultInstructions: "Stay clear of all engine and propulsion components. Never enter the water while engines are running or in gear. Wait for explicit all-clear from the captain.", compliance_target: "all" },
  ],
};
