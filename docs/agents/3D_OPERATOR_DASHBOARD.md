# DockPass — Phase 3D Agent Instructions
...
export async function generateManifest(
  ...
  doc.setAuthor('DockPass')
  doc.setCreator('DockPass — dockpass.io')
  ...
  page.drawText('DockPass — dockpass.io', {
    x: width - 200, y: y - 20,
  ...
  finalPage.drawText(
    `Generated by DockPass (dockpass.io) · ${new Date().toLocaleString('en-US')} · Keep a copy for USCG compliance`,
...
TEST 8 — Captain snapshot:
  ...
  URL appears: dockpass.io/snapshot/[token]
...
TEST 9 — WhatsApp message copy:
  ...
  Clipboard contains message with:
    Trip link (dockpass.io/trip/[slug])
    Trip code (*CODE*)
    Boat name
